$(function(){
	$('.pbtn').click(function(){
		...
		$('#pfrm').attr('action', '/cls/reBoard1/reBoardList.cls');
	...
	$('.ebtn').click(function(){
		...
		*/
		...
		$('#frm').attr('action', '/cls/reBoard1/reBoardEditView.cls');
		$('#frm').submit();
	...
	$('#wrbtn').click(function(){
		...
		// 데이터 읽고
		var txt = $('#body').val();
		var url = '/cls/reBoard1/reBoardWriteProc.cls';
		if(str != 'edit'){
			if(!txt){
				alert('# 메세지를 입력하세요!');
				return;
			...
			url = '/cls/reBoard1/reBoardEditProc.cls';
		}
	...
	$('.rebtn').click(function(){
		...
		// form 태그 속성 셋팅
		$('#frm').attr('method', 'POST');
		$('#frm').attr('action', '/cls/reBoard1/reBoardComment.cls');
		$('#frm').submit();
	});
